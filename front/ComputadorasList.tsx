...
import { ComputadoraPerifericosSection } from './ComputadoraPerifericosSection';
import { 
  ...
  ChevronLeft, Trash2, Calendar, User, Info, HardDrive, ShieldAlert, Plus, UserCheck, X,
  Usb, SlidersHorizontal, Layers
} from 'lucide-react';
...
export default function ComputadorasList({ computers, onUpdateComputer, onAddComputer, onRefreshTelemetry }: ComputadorasListProps) {
  ...
  const [activeTab, setActiveTab2] = useState<'hardware' | 'perifericos' | 'software' | 'custodia'>('hardware');
  ...
  // Quick edit modal states for Custody and Lifecycle management (Friendly UX)
  const [quickEditComp, setQuickEditComp] = useState<AgentComputer | null>(null);
  const [quickAssignee, setQuickAssignee] = useState('');
  const [quickLocation, setQuickLocation] = useState('');
  const [quickStatus, setQuickStatus] = useState('');
  const [quickMotive, setQuickMotive] = useState('');

  useEffect(() => {
    if (quickEditComp) {
      setQuickAssignee(quickEditComp.responsable_inventario || '');
      setQuickLocation(quickEditComp.ubicacion || 'CAPITAL_HUMANO');
      setQuickStatus(quickEditComp.estado_it || 'Asignada');
      setQuickMotive('');
    }
  }, [quickEditComp]);
  ...
  // Unified Quick Edit Form Submission for Custody & State transitions
  const handleSaveQuickEdit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!quickEditComp) return;

    const stateChanged = quickEditComp.estado_it !== quickStatus;
    if (stateChanged && !quickMotive.trim()) {
      alert('Debe especificar un motivo detallado obligatoriamente para cambiar el estado.');
      return;
    }

    let nextHistory = quickEditComp.historial_estados ? [...quickEditComp.historial_estados] : [];
    if (stateChanged) {
      const nowStr = new Date().toLocaleDateString('es-ES', { day: 'numeric', month: 'long', year: 'numeric' }) + `, ${new Date().toLocaleTimeString('es-ES', { hour: '2-digit', minute: '2-digit' })}`;
      const historyEntry = {
        fecha: nowStr,
        autor: 'Daniel Ortega (IT Admin)',
        estado_anterior: quickEditComp.estado_it || 'Asignada',
        estado_nuevo: quickStatus,
        motivo: quickMotive
      };
      nextHistory = [historyEntry, ...nextHistory];
    }

    let finalAssignee = quickAssignee.trim();
    let finalStatus = quickStatus;

    // Automatic smart adjustments to avoid state mismatches
    if (finalAssignee && (finalStatus === 'Disponible' || finalStatus === 'Retirada')) {
      finalStatus = 'Asignada';
    } else if (!finalAssignee && finalStatus === 'Asignada') {
      finalStatus = 'Disponible';
    }

    const updated: AgentComputer = {
      ...quickEditComp,
      responsable_inventario: finalAssignee || undefined,
      ubicacion: quickLocation,
      estado_it: finalStatus,
      historial_estados: nextHistory
    };

    onUpdateComputer(updated);
    setQuickEditComp(null);
    alert(`Asignaciones de "${quickEditComp.hostname}" actualizadas con éxito.`);
  };
  ...
  return (
    <div id="computadoras-telemetry-view" className="space-y-6">
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200 text-[10px] font-bold text-slate-500 uppercase tracking-wider">
                    <th className="py-3 px-4">EQUIPO / HOSTNAME</th>
                    <th className="py-3 px-4">IDENTIFICADORES</th>
                    <th className="py-3 px-4">UBICACIÓN</th>
                    <th className="py-3 px-4">CUSTODIO (RESPONSABLE)</th>
                    <th className="py-3 px-4 text-center">ESTADO IT</th>
                ...
                <tbody className="divide-y divide-slate-100 text-xs">
                  {unifiedFilteredComputers.length > 0 ? (
                    unifiedFilteredComputers.map((c) => {
                      const isOnline = c.estado_conexion === 'ONLINE';
                        ...
                        >
                          ...
                          <td className="py-3 px-4">
                            <div className="flex items-center gap-3">
                              <span className={`p-2 rounded-lg ${isOnline ? 'bg-blue-50 text-blue-600' : 'bg-slate-100 text-slate-400'}`}>
                                <Laptop className="w-4 h-4" />
                              </span>
                              <div>
                                <div className="font-bold text-slate-900 flex items-center gap-2">
                                  <span>{c.hostname}</span>
                                  <span className={`w-1.5 h-1.5 rounded-full inline-block ${isOnline ? 'bg-emerald-500 animate-pulse' : 'bg-slate-300'}`} />
                                </div>
                                <span className="text-[10px] text-slate-400">{c.sistema_operativo}</span>
                              </div>
                            </div>
                          </td>
                          ...
                          <td className="py-3 px-4">
                            <div className="space-y-1">
                              <div className="flex items-center gap-1.5" onClick={(e) => e.stopPropagation()}>
                                <button 
                                  onClick={(e) => handleCopyAnydesk(c.anydesk_id, e)}
                                  className="font-mono bg-slate-100 hover:bg-slate-200 px-2 py-0.5 rounded text-[10px] font-bold text-slate-700 flex items-center gap-1 cursor-pointer transition-colors border border-slate-200"
                                  title="Copiar ID de AnyDesk"
                                >
                                  <span>ID: {c.anydesk_id}</span>
                                  <Copy className="w-3 h-3 text-slate-400" />
                                </button>
                                {copiedId === c.anydesk_id && (
                                  <span className="text-[9px] text-emerald-600 font-bold font-mono">¡Copiado!</span>
                                )}
                              </div>
                              <div className="text-[10px] text-slate-400 font-mono">
                                UUID: <span className="text-slate-500">{c.uuid.substring(0, 8)}...</span>
                              </div>
                            </div>
                          </td>
                          ...
                          {/* Location */}
                          <td className="py-3 px-4">
                            <span className="text-[10px] font-bold text-slate-600 bg-slate-100 px-2 py-0.5 rounded border border-slate-200 uppercase tracking-wide">
                          ...
                          <td className="py-3 px-4">
                            {c.responsable_inventario ? (
                              <div className="flex items-center gap-1.5 text-slate-700">
                                <div className="w-5 h-5 rounded bg-indigo-100 text-indigo-700 flex items-center justify-center text-[9px] font-bold uppercase shrink-0">
                                  {c.responsable_inventario.split(' ').map(n => n[0]).join('').substring(0, 2)}
                                </div>
                                <span className="font-bold text-slate-800 text-xs">{c.responsable_inventario}</span>
                              </div>
                            ) : (
                              <span className="text-[11px] font-semibold text-slate-400 italic">Disponible en Almacén</span>
                            )}
                          </td>
                          ...
                          <td className="py-3 px-4 text-center">
                            {c.estado_it === 'Asignada' || !c.estado_it ? (
                              <span className="inline-flex px-2 py-0.5 rounded-full text-[10px] font-extrabold bg-indigo-50 border border-indigo-200 text-indigo-700">Asignada</span>
                            ) : c.estado_it === 'Disponible' ? (
                              <span className="inline-flex px-2 py-0.5 rounded-full text-[10px] font-extrabold bg-emerald-50 border border-emerald-200 text-emerald-700">Disponible</span>
                            ) : c.estado_it === 'En Reparación' ? (
                              <span className="inline-flex px-2 py-0.5 rounded-full text-[10px] font-extrabold bg-amber-50 border border-amber-200 text-amber-700">Reparación</span>
                            ) : (
                              <span className="inline-flex px-2 py-0.5 rounded-full text-[10px] font-extrabold bg-rose-50 border border-rose-200 text-rose-700">De baja</span>
                            )}
                          </td>
                          ...
                          <td className="py-3 px-4 text-right" onClick={(e) => e.stopPropagation()}>
                            <button
                              onClick={() => setQuickEditComp(c)}
                              className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-blue-50 hover:bg-blue-100 text-blue-600 font-extrabold text-xs rounded-lg transition-colors cursor-pointer border border-blue-100"
                              title="Gestionar Asignación y Ciclo de Vida"
                            >
        ...
        {selectedComp && (
          <div className="fixed inset-0 z-50 bg-slate-900/45 flex items-center justify-center p-4 overflow-y-auto">
            ...
            <div className="relative bg-[#f8fafc] rounded-xl border border-slate-250 shadow-2xl w-full max-w-5xl overflow-hidden flex flex-col max-h-[92vh]">
              ...
              <div className="flex border-b border-slate-250 bg-white px-6 shrink-0 gap-1 overflow-x-auto">
                <button 
                  onClick={() => setActiveTab2('hardware')}
                  className={`py-3 px-3 text-xs font-bold border-b-2 transition-all shrink-0 flex items-center gap-1.5 ${activeTab === 'hardware' ? 'border-blue-600 text-blue-600 font-extrabold' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
                >
                  <Cpu className="w-3.5 h-3.5" />
                  Hardware
                </button>
                <button 
                  onClick={() => setActiveTab2('perifericos')}
                  className={`py-3 px-3 text-xs font-bold border-b-2 transition-all shrink-0 flex items-center gap-1.5 ${activeTab === 'perifericos' ? 'border-blue-600 text-blue-600 font-extrabold' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
                >
                  <SlidersHorizontal className="w-3.5 h-3.5" />
                  Periféricos & Conexiones
                  {selectedComp.perifericos && (
                    <span className="px-1.5 py-0.2 rounded-full text-[10px] bg-blue-100 text-blue-700 font-mono font-bold">
                      {(selectedComp.perifericos.monitores?.length || 0) + (selectedComp.perifericos.impresoras?.length || 0) + (selectedComp.perifericos.dispositivos_usb?.length || 0)}
                    </span>
                  )}
                </button>
                <button 
                  onClick={() => setActiveTab2('software')}
                  className={`py-3 px-3 text-xs font-bold border-b-2 transition-all shrink-0 flex items-center gap-1.5 ${activeTab === 'software' ? 'border-blue-600 text-blue-600 font-extrabold' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
                >
                  <ShieldCheck className="w-3.5 h-3.5" />
                  Software & Seguridad
                </button>
                <button 
                  onClick={() => setActiveTab2('custodia')}
                  className={`py-3 px-3 text-xs font-bold border-b-2 transition-all shrink-0 flex items-center gap-1.5 ${activeTab === 'custodia' ? 'border-blue-600 text-blue-600 font-extrabold' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
                >
                  <UserCheck className="w-3.5 h-3.5" />
                  Inventario & Custodia
                </button>
              ...
              <div className="p-6 overflow-y-auto space-y-6 flex-1 bg-slate-50 max-h-[64vh]">
                ...
                {activeTab === 'hardware' && (
                  <div className="space-y-6">
                    ...
                    {/* Quick Peripheral summary teaser banner */}
                    <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-3xs flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[11px] text-slate-500 mt-0.5">
                            {selectedComp.perifericos?.monitores?.length || 0} monitor(es) • {selectedComp.perifericos?.impresoras?.length || 0} impresora(s) • {selectedComp.perifericos?.dispositivos_usb?.length || 0} periférico(s) USB
                          </p>
                        </div>
                      </div>
                      <button
      ...
      {/* Quick Edit Modal for Custody & State Transitions */}
      <AnimatePresence>
        {quickEditComp && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="bg-white rounded-2xl border border-slate-200 shadow-2xl max-w-md w-full overflow-hidden relative z-10"
            >
              <div className="p-5 bg-slate-50 border-b border-slate-100 flex items-center justify-between">
                <div>
                  <h3 className="font-extrabold text-slate-900 text-sm flex items-center gap-2">
                    <UserCheck className="w-4 h-4 text-blue-600" />
                    <span>Gestionar Custodia y Ciclo de Vida</span>
                  </h3>
                  <p className="text-[10px] text-slate-500 font-mono mt-0.5">
                    {quickEditComp.hostname} • UUID: {quickEditComp.uuid.substring(0, 8)}
                  </p>
                </div>
                <button
                  type="button"
              ...
              <form onSubmit={handleSaveQuickEdit} className="p-5 space-y-4">
                ...
                <div className="space-y-1.5">
                  <label className="block text-[11px] font-bold text-slate-600 uppercase tracking-wider">
                    Custodio / Responsable de Uso
                  </label>
                  <div className="relative">
                    <User className="absolute left-3 top-2.5 w-4 h-4 text-slate-400" />
                    <input
                      type="text"
                      placeholder="Nombre del personal (ej: Agustina López)"
                      value={quickAssignee}
                      onChange={(e) => setQuickAssignee(e.target.value)}
                      className="pl-9 pr-3 py-2 bg-slate-50 border border-slate-200 hover:border-slate-300 focus:border-blue-500 focus:bg-white text-xs font-semibold rounded-lg transition-all text-slate-800 w-full"
                    />
                  </div>
                  <p className="text-[9px] text-slate-400">
                    Al asignar un custodio, el estado cambiará automáticamente a <strong className="text-slate-500 font-bold">Asignada</strong>. Si se borra, volverá a <strong className="text-slate-500 font-bold">Disponible</strong>.
                  </p>
                </div>
                ...
                <div className="space-y-1.5">
                  <label className="block text-[11px] font-bold text-slate-600 uppercase tracking-wider">
                    Ubicación / Área Operativa
                  </label>
                  <select
                    value={quickLocation}
                    onChange={(e) => setQuickLocation(e.target.value)}
                    className="px-3 py-2 bg-slate-50 border border-slate-200 hover:border-slate-300 focus:border-blue-500 focus:bg-white text-xs font-bold rounded-lg transition-all text-slate-800 w-full cursor-pointer"
                  >
                    <option value="CAPITAL_HUMANO">Recursos Humanos (Capital Humano)</option>
                    <option value="SOPORTE_IT">Soporte IT / Sistemas</option>
                    <option value="VENTAS_CALL">Ventas / Call Center</option>
                    <option value="ADMINISTRACION">Administración / Contabilidad</option>
                    <option value="MONITOREO_CCTV">Monitoreo CCTV / Seguridad</option>
                    <option value="DIRECCION">Dirección / Junta Directiva</option>
                  </select>
                </div>
                ...
                <div className="space-y-1.5">
                  <label className="block text-[11px] font-bold text-slate-600 uppercase tracking-wider">
                    Estado Técnico de Ciclo de Vida
                  </label>
                  <select
                    value={quickStatus}
                  ...
                  >
                    <option value="Asignada">Asignada (En uso activo)</option>
                    <option value="Disponible">Disponible (En bodega listo para uso)</option>
                    <option value="En Reparación">En Reparación (Mantenimiento o cambio de hardware)</option>
                    <option value="Retirada">Retirada (Dar de baja / descompuesta)</option>
                  </select>
                </div>

                {/* Motivo (Conditional input) */}
                {quickStatus !== quickEditComp.estado_it && (
                  <motion.div
                  ...
                  >
                    <label className="block text-[11px] font-extrabold text-rose-600 uppercase tracking-wider flex items-center gap-1">
                      <ShieldAlert className="w-3.5 h-3.5" />
                      <span>Justificación Técnica (Obligatoria)</span>
                    </label>
                    <textarea
                      required
                      placeholder="Especifique el motivo técnico del cambio de estado..."
                      value={quickMotive}
                      onChange={(e) => setQuickMotive(e.target.value)}
                      rows={2}
                      className="px-3 py-2 bg-rose-50/20 border border-rose-200 focus:border-rose-500 focus:bg-white text-xs font-medium rounded-lg transition-all text-slate-800 w-full resize-none"
                    />
                  </motion.div>
                )}
                ...
                <div className="pt-3 border-t border-slate-100 flex items-center justify-end gap-2">
                  <button
                    type="button"
                    onClick={() => setQuickEditComp(null)}
                    className="px-4 py-2 hover:bg-slate-100 text-slate-600 font-bold text-xs rounded-lg transition-colors cursor-pointer"
                  >
                    Cancelar
                  </button>
                  <button
                    type="submit"
                    className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-extrabold text-xs rounded-lg transition-colors cursor-pointer shadow-md shadow-blue-500/10 flex items-center gap-1.5"
                  >
