    ...
    <AnimatePresence>
      <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-[9999]">
        <motion.div
          ...
          className="bg-white border border-slate-200 rounded-xl max-w-2xl w-full overflow-hidden shadow-2xl border-t-[3px] border-t-accent"
        >
          <div className="px-6 py-4 border-b border-slate-150 bg-slate-50 flex items-center justify-between">
            <div className="flex items-center gap-2.5 min-w-0">
              ...
              {isEdit ? <Edit2 className="w-5 h-5 text-blue-600 shrink-0" /> : <Server className="w-5 h-5 text-emerald-600 shrink-0" />}
                ...
                <div key={field.name} className={`space-y-1 ${field.fullWidth ? 'sm:col-span-2' : ''}`}>
                  ...
                  {field.type === 'select' ? (
                    <FriendlySelect
                      ...
                      onChange={(next) => onChange({ target: { name: field.name, value: next } })}
                    />
                  ) : field.type === 'textarea' ? (
                    <textarea
                      name={field.name}
