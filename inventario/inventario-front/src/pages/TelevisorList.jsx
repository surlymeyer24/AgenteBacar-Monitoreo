...
import { Tv2, Search, Edit2, Trash2, X, ShieldCheck } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import ImportModal from '../components/ImportModal';
import { televisoresSchema } from '../lib/importSchemas/televisoresSchema';
import {
  fetchTelevisores,
  crearTelevisor,
  actualizarTelevisor,
...
export default function TelevisorList() {
  ...
  function cargarLista() {
    ...
    fetchTelevisores()
      ...
      .catch(() => setError('No se pudo cargar el inventario de televisores.'))
  ...
  const areas = useMemo(() => {
    const set = new Set(lista.map(t => t.area).filter(Boolean));
  ...
  const dataset = useMemo(() => {
    ...
    return lista.filter(t => {
      ...
      if (!q) return true;
      return [t.marca, t.modelo, t.numeroSerie, t.area, t.direccionIp, t.estado]
        .filter(Boolean)
  ...
  async function handleDelete(item) {
    ...
    if (!window.confirm(`¿Eliminar televisor "${label}"?`)) return;
    try {
      await deleteTelevisor(item.id);
      showBanner('Televisor eliminado.');
      cargarLista();
    } catch {
      window.alert('No se pudo eliminar el televisor.');
    }
  }

  async function handleSubmit(e) {
    e.preventDefault();
    if (!form.marca.trim() || !form.area.trim()) {
      setFormError('Marca y área son obligatorios.');
      return;
    }
    const body = {
      ...
      estado: normalizarEstadoTelevisor(form.estado),
    };
    setGuardando(true);
    setFormError('');
    try {
      if (isEdit) {
        await actualizarTelevisor(editingId, body);
        showBanner('Televisor actualizado.');
      } else {
        await crearTelevisor(body);
        showBanner('Televisor registrado.');
      }
  ...
  async function handleImport(rows) {
    ...
    for (const row of rows) {
      if (!row.marca?.trim() || !row.area?.trim()) continue;
      try {
        await crearTelevisor({
          ...
          estado: normalizarEstadoTelevisor(row.estado),
        });
        ok += 1;
      } catch {
        fail += 1;
  ...
  if (cargando) return <StudioLoading message="Cargando televisores…" />;
  if (error) return <StudioError message={error} />;

  return (
    <StudioPageShell
      title={`Televisores (${lista.length})`}
      subtitle="Pantallas en salas, recepción y áreas comunes."
      actions={
        <>
    ...
    >
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm space-y-3 mb-4">
        <div className="flex flex-col md:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Buscar por marca, modelo, serie, área, IP…"
              value={searchTerm}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div className="space-y-2">
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h3 className="font-extrabold text-slate-900 text-base">{tv.marca}</h3>
                    {tv.modelo ? <p className="text-sm text-slate-500">{tv.modelo}</p> : null}
                  </div>
                </div>
              ...
              <div className="text-sm text-slate-600 bg-slate-50 p-2.5 rounded-lg border border-slate-100 space-y-1">
                <p><strong>Área:</strong> {tv.area || '—'}</p>
                <p><strong>IP:</strong> <span className="font-mono">{tv.direccionIp || '—'}</span></p>
                <p><strong>Serie:</strong> {tv.numeroSerie || '—'}</p>
              </div>
            </div>
            <div className="pt-2 border-t border-slate-100 flex gap-2">
              <button
                type="button"
                onClick={() => abrirEditar(tv)}
        ...
        {dataset.length === 0 ? (
          <div className="col-span-full bg-slate-50 rounded-xl border border-dashed border-slate-200 p-12 text-center text-slate-400 italic text-sm">
            No hay televisores registrados o no coinciden con los filtros.
          </div>
        ) : null}
      </div>
            ...
            >
              <div className="px-6 py-4 border-b border-slate-150 bg-slate-50 flex items-center justify-between">
                <div className="flex items-center gap-2.5 min-w-0">
                  ...
                  <Tv2 className="w-5 h-5 text-blue-600 shrink-0" />
                  <span className="font-extrabold text-sm text-slate-900 uppercase tracking-wide">
                    {isEdit ? 'Editar televisor' : 'Registrar televisor'}
                  </span>
                </div>
                <button
                  type="button"
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      placeholder="Ej. Recepción, Sala reuniones"
                      className="w-full p-2 border rounded-lg bg-slate-50 focus:ring-1 focus:ring-blue-600 font-normal"
                    />
                  </div>
                  <div className="space-y-1">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      onChange={e => setForm(f => ({ ...f, direccionIp: e.target.value }))}
      ...
      <ImportModal
        ...
        schema={televisoresSchema}
        entityName="Televisores"
